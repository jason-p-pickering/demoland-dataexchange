...
We will load a few helper libraries, and to define the login details for the two systems.
...
Since the data has been requested in  monthly format, we will need to aggregate it to quarters and then calculate the number of negative tests from the total tests performed and the positive tests.
...
htc.data<-reshape2::recast(htc.data, orgUnit + period ~ dataElement,fun.aggregate=function(x) {sum(x,na.rm=T)})
...
#Lets get the data by month, to show how we can aggregate monthly data to quarterly data, getting all data for Fiscal Year 2014
...
names(htc.data)[names(htc.data) == "Period"]<-"period"
```
...
```{r}
#Copy the period to a new column 
htc.data$quarter<-htc$metaData$period
...
paste0(year,sprintf("%02d",seq(end-2,end)))
}
```
...
other<-htc.data[i,c("dataElement","orgUnit")]
vals<-monthlyValuesFromQuarters(as.numeric(as.character(htc.data$value[i])))
period<-monthlyPeriodsFromQuarters(htc.data$period[i])
foo<-merge(other,vals,all=T)
names(foo)<-c("dataElement","orgUnit","value")
foo<-cbind(foo,period)
data.months<-rbind(data.months,foo[names(data.months)])
}
```
...
attribs<-list(orgUnit=as.character(data.months$source_id[i]),
#We must correctly format the period to correspond to what ADX requires
period=paste0(substr(as.character(data.months$period[i]),0,4),"-",substr(as.character(data.months$period[i]),5,6),"-01/P1M"),
